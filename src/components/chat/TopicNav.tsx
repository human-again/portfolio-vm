  ...
  return (
    <div className="flex items-center justify-center gap-2 overflow-x-auto scrollbar-none pb-1">
      {topics.map((topic) => (
        ...
        >
          <topic.icon size={14} className="text-muted-foreground" />
          <span className="font-medium text-foreground">{topic.label}</span>
        </Link>
      ))}
      <button className="flex items-center justify-center w-9 h-9 rounded-full border border-border bg-white hover:bg-muted transition-colors shrink-0">
